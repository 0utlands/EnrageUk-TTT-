...
function PANEL:UpdatePlayerData()
   ...
   if self.cols[4] then
	...
	local red = ply:GetNWInt("SB_rankcolor_r", 255)
	local green = ply:GetNWInt("SB_rankcolor_g", 255)
	local blue = ply:GetNWInt("SB_rankcolor_b", 255)
	local alpha = ply:GetNWInt("SB_rankcolor_a", 255)
   
      self.cols[5]:SetText(ply:GetNWString("SB_ranktext") or self.Player:GetUserGroup():gsub("^%l", string.upper))
   self.cols[5]:SetTextColor(Color(red,green,blue,alpha))
...
function PANEL:LayoutColumns()
   for k,v in ipairs(self.cols) do
	  ...
	  if k == 5 then
		v:SizeToContents()
		v:SetPos(self:GetWide() - (75*k) - v:GetWide()/2, (SB_ROW_HEIGHT - v:GetTall()) / 2)
	  end
   end
