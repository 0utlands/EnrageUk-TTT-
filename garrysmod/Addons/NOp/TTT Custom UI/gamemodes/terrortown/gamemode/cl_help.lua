...
function HELPSCRN:Show()
   ...
   local dclose = vgui.Create("DButton", dframe)
   local bw, bh = 50, 25
   dclose:SetSize(bw, bh)
   dclose:SetPos(w - bw - margin, h - bh - margin/2)
   dclose:SetText(GetTranslation("close"))
   dclose:SetTextColor( tttreskin.hsclosebtntext )
   dclose.OnCursorEntered = function(self)
    self:SetTextColor( tttreskin.hsclosebtntexthover )
  end
  dclose.OnCursorExited = function(self)
    self:SetTextColor( tttreskin.hsclosebtntext )
  end
  dclose.Paint = function(self,w,h)
    draw.RoundedBox( 0, 0, 0, w, h, tttreskin.hsclosebtnborder )
    draw.RoundedBox( 0, 1, 1, w-2, h-1, tttreskin.hsclosebtngrdtop )

    surface.SetDrawColor( tttreskin.hsclosebtngrdbtm )
    surface.SetTexture( surface.GetTextureID( "gui/gradient" ) )
    surface.DrawTexturedRectRotated( w/2, h/2, h-1, w-2, 90 )

    draw.RoundedBox( 0, 1, 1, w-2, 1, tttreskin.hsclosebtnshine )
   end
   dclose.DoClick = function() dframe:Close() end
   ...
   dtabs.Paint = function(self,w,h)
    draw.RoundedBox( 0, 0, 0, w, h, tttreskin.hsbody )
 end
      ...
      if (!v.Tab) then continue end
        ...
        v.Tab.Paint = function(self,w,h)
          if v.Tab == dtabs:GetActiveTab() then
            draw.RoundedBox(0, 0, 0, w, h, tttreskin.hsbodytabselected)
          else
            draw.RoundedBox(0, 0, 0, w, h, tttreskin.hsbodytab)
          end
      end
    end
...
function HELPSCRN:CreateTutorial(parent)
   ...
   bnext:CopyPos(bar)
   bnext:AlignRight(1)
   bnext:SetTextColor( tttreskin.hsnextbtntext )
   bnext.OnCursorEntered = function(self)
    self:SetTextColor( tttreskin.hsnextbtntexthover )
  end
  bnext.OnCursorExited = function(self)
    self:SetTextColor( tttreskin.hsnextbtntext )
  end
  bnext.Paint = function(self,w,h)
    draw.RoundedBox( 0, 0, 0, w, h, tttreskin.hsnextbtnborder )
    draw.RoundedBox( 0, 1, 1, w-2, h-2, tttreskin.hsnextbtngrdtop )

    surface.SetDrawColor( tttreskin.hsnextbtngrdbtm )
    surface.SetTexture( surface.GetTextureID( "gui/gradient" ) )
    surface.DrawTexturedRectRotated( w/2, h/2, h-2, w-2, 90 )

    draw.RoundedBox( 0, 1, 1, w-2, 1, tttreskin.hsnextbtnshine )
   end
   ...
   bprev:CopyPos(bar)
   bprev:AlignLeft()
   bprev:SetTextColor( tttreskin.hsprevbtntext )
   bprev.OnCursorEntered = function(self)
    self:SetTextColor( tttreskin.hsprevbtntexthover )
  end
  bprev.OnCursorExited = function(self)
    self:SetTextColor( tttreskin.hsprevbtntext )
  end
  bprev.Paint = function(self,w,h)
    draw.RoundedBox( 0, 0, 0, w, h, tttreskin.hsprevbtnborder )
    draw.RoundedBox( 0, 1, 1, w-2, h-2, tttreskin.hsprevbtngrdtop )

    surface.SetDrawColor( tttreskin.hsprevbtngrdbtm )
    surface.SetTexture( surface.GetTextureID( "gui/gradient" ) )
    surface.DrawTexturedRectRotated( w/2, h/2, h-2, w-2, 90 )

    draw.RoundedBox( 0, 1, 1, w-2, 1, tttreskin.hsprevbtnshine )
   end
