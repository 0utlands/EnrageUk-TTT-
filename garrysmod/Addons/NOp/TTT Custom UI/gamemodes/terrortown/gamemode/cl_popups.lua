...
local function IdlePopup()
   ...
   dframe.Paint = function(self,w,h)
    draw.RoundedBox( 0, 0, 0, w, h, tttreskin.imbg )
    draw.RoundedBox( 0, 0, 0, w, 25, tttreskin.imbgtop )
   end
   ...
   cancel:SetTextColor( tttreskin.imclosebtntext )
   cancel.OnCursorEntered = function(self)
    self:SetTextColor( tttreskin.imclosebtntexthover )
  end
  cancel.OnCursorExited = function(self)
    self:SetTextColor( tttreskin.imclosebtntext )
  end
  cancel.Paint = function(self,w,h)
    draw.RoundedBox( 0, 0, 0, w, h, tttreskin.imclosebtnborder )
    draw.RoundedBox( 0, 1, 1, w-2, h-2, tttreskin.imclosebtngrdtop )

    surface.SetDrawColor( tttreskin.imclosebtngrdbtm )
    surface.SetTexture( surface.GetTextureID( "gui/gradient" ) )
    surface.DrawTexturedRectRotated( w/2, h/2, h-1, w-2, 90 )

    draw.RoundedBox( 0, 1, 1, w-2, 1, tttreskin.imclosebtnshine )
   end
   ...
   disable:SetTextColor( tttreskin.imdisablebtntext )
   disable.OnCursorEntered = function(self)
    self:SetTextColor( tttreskin.imdisablebtntexthover )
  end
  disable.OnCursorExited = function(self)
    self:SetTextColor( tttreskin.imdisablebtntext )
  end
  disable.Paint = function(self,w,h)
    draw.RoundedBox( 0, 0, 0, w, h, tttreskin.imdisablebtnborder )
    draw.RoundedBox( 0, 1, 1, w-2, h-2, tttreskin.imdisablebtngrdtop )

    surface.SetDrawColor( tttreskin.imdisablebtngrdbtm )
    surface.SetTexture( surface.GetTextureID( "gui/gradient" ) )
    surface.DrawTexturedRectRotated( w/2, h/2, h-1, w-2, 90 )

    draw.RoundedBox( 0, 1, 1, w-2, 1, tttreskin.imdisablebtnshine )
   end
