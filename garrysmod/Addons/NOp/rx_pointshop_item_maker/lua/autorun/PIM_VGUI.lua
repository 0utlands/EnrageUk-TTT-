...
if CLIENT then
	...
	concommand.Add("ps_PIMEditor",function(ply,cmd,args)
		PIM_OpenEditor()
	end)
...
function PANEL:Paint()
		if !self.MainModel then return end
			...
			if self.ModelSpecial_Spin.Enable == 1 then
				self.ModelDrawingAngle.p = (CurTime() * self.ModelSpecial_Spin.Angle.p*90)
				self.ModelDrawingAngle.y = (CurTime() * self.ModelSpecial_Spin.Angle.y*90)
				self.ModelDrawingAngle.r = (CurTime() * self.ModelSpecial_Spin.Angle.r*90)
			else
			...
			Ang:RotateAroundAxis(Ang:Forward(), (self.ModelDrawingAngle.p))
			Ang:RotateAroundAxis(Ang:Up(), (self.ModelDrawingAngle.y))
			Ang:RotateAroundAxis(Ang:Right(), (self.ModelDrawingAngle.r))
			...
			-- Special : Halo
			if self.ModelSpecial_Halo.Enable == 1 then
				halo.Add( {self.MainModel},
				self.ModelSpecial_Halo.Color,
				self.ModelSpecial_Halo.Size, 
				self.ModelSpecial_Halo.Size,
				1)
			end
end
