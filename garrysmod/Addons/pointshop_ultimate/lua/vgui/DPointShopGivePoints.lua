...
function PANEL:Init()
	...
	self:SetDrawOnTop(true)
			...
			TopBar.Paint = function(slf)
				surface.SetDrawColor( UPS_Config.BGCol.GP_TitleBG )
				surface.DrawRect(0, 0, slf:GetWide(), slf:GetTall())
				surface.SetDrawColor( UPS_Config.Col.PG.Main_OutLine )
				surface.DrawRect(0, slf:GetTall()-1, slf:GetWide(), 1)
				draw.SimpleText("Give Points", 'RXF_Treb_S22', 10, 2, UPS_Config.Col.PG.Main_TitleText)
			end

				local Button = vgui.Create("UPS_DSWButton",TopBar)
...
function PANEL:Paint()
		surface.SetDrawColor(UPS_Config.BGCol.GP_BodyBG)
